...
class ArrangeModel(InpaintModel):
    ...
    def forward(self, data, mode):
        ...
        if mode == 'generator':
            g_loss, composed_image, composed_image_aux, recon_aux = self.compute_generator_loss(inputs, real_image, mask)
            generated = {
            ...
            with torch.no_grad():
                coarse_image, fake_image, aux_image, recon_aux = self.generate_fake(inputs, real_image, mask)
    ...
    def compute_discriminator_loss(self, inputs, real_image, mask):
        ...
        if not self.opt.no_gan_loss:
            ...
            D_losses['D_Fake_aux'] = self.criterionGAN(pred_fake, False, for_discriminator=True)
            D_losses['D_real_aux'] = self.criterionGAN(pred_real, True, for_discriminator=True)
            self.netD = _netD
